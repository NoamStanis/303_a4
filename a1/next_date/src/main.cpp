...
void validateInput(SimpleDate &sd) {
    ...
    if(isLeapYear) {
        if(month == 2 && day > 29) {
            e = new DateException("Invalid leap year date");
            throw e;
        }
    }

    else if(month == 2 && day > 28) {
        e = new DateException("Invalid February date");
        throw e;

    }

    if(year < 1812 || year > 2030) {
        e =  new DateException("Invalid Year");
        throw e;
    }

    if(month < 1 || month > 12) {
        e = new DateException("Invalid Month");
        throw e;
    }
    ...
    if(monthCheck != std::end(months30)) {
        if(day < 1 || day > 30) {
            e = new DateException("Invalid Day");
            throw e;
        }
    }
    //31 days
